...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Download ZIP para deploy no Railway
  app.get('/cartao-vidah-railway.zip', (req, res) => {
    const path = require('path');
    const zipPath = path.join(process.cwd(), 'cartao-vidah-railway.zip');
    
    res.download(zipPath, 'cartao-vidah-railway.zip', (err) => {
      if (err) {
  ...
  // Download ZIP completo para migração GitHub
  app.get('/cartao-vidah-complete-migration.zip', (req, res) => {
    const path = require('path');
    const zipPath = path.join(process.cwd(), 'cartao-vidah-complete-migration.zip');
    
    res.download(zipPath, 'cartao-vidah-complete-migration.zip', (err) => {
      if (err) {
        console.error('Erro ao baixar ZIP:', err);
        res.status(404).json({ error: 'Arquivo não encontrado' });
      }
  ...
  // Download ZIP final limpo
  app.get('/cartao-vidah-final.zip', (req, res) => {
    const path = require('path');
    const zipPath = path.join(process.cwd(), 'cartao-vidah-final.zip');
    
    res.download(zipPath, 'cartao-vidah-final.zip', (err) => {
      if (err) {
        console.error('Erro ao baixar ZIP:', err);
        res.status(404).json({ error: 'Arquivo não encontrado' });
      }
